...
class CenterofMass(Layer):
    ...
    def build(self, input_shape):
        self.COM_weightXL = self.add_weight(name='COM_weightXL', shape=(self.size, 1),
                                            initializer=ini.Constant(value=0.25), trainable=False)
        self.COM_weightXR = self.add_weight(name='COM_weightXR', shape=(self.size, 1),
                                            initializer=ini.Constant(value=0.25), trainable=False)
        self.COM_weightYL = self.add_weight(name='COM_weightYL', shape=(self.size, 1),
                                            initializer=ini.Constant(value=0.25), trainable=False)
        self.COM_weightYR = self.add_weight(name='COM_weightYR', shape=(self.size, 1),
                                            initializer=ini.Constant(value=0.25), trainable=False)
        self.COM_weightZL = self.add_weight(name='COM_weightZL', shape=(self.size, 1),
                                            initializer=ini.Constant(value=0.25), trainable=False)
        self.COM_weightZR = self.add_weight(name='COM_weightZR', shape=(self.size, 1),
                                            initializer=ini.Constant(value=0.25), trainable=False)
